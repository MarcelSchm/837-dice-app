...
- New version: bump `versionCode`/`versionName` in `app/build.gradle.kts`,
  regenerate `CHANGELOG.md` (`git cliff --tag vX.Y -o CHANGELOG.md`, config
  in `cliff.toml`), commit, `git tag vX.Y && git push origin main vX.Y`.
- Release notes are generated by git-cliff from Conventional Commits in CI
  (orhun/git-cliff-action) - another reason the commit convention is
  mandatory. Checkout uses fetch-depth 0 for that.
