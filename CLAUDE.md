...
- New version: bump `versionCode`/`versionName` in `app/build.gradle.kts`,
  commit, `git tag vX.Y && git push origin main vX.Y`. CI generates the
  release notes AND pushes a regenerated `CHANGELOG.md` to main as a
  `chore(release)` bot commit (skipped by cliff.toml), so pull main after
  releasing.
